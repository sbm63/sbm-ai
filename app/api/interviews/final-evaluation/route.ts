...
export async function POST(req: NextRequest) {
  try {
    ...
    const { rows: interviewRows } = await executeWithRetry(
      () => db.sql`
...
Please provide a comprehensive final evaluation of this interview.`,
    ...
    try {
      ...
      finalEvaluation = {
        ...
        recommendation:
          avgScore >= 7 ? 'hire' : avgScore >= 5 ? 'maybe' : 'reject',
        summary:
          'Interview completed successfully. Detailed analysis available.',
        detailedFeedback: {
    ...
    return NextResponse.json({
      ...
      interviewStats: {
        ...
        averageScore:
          responses.reduce(
            (acc: number, qa: any) => acc + (qa.evaluation?.score || 5),
            0,
          ) / responses.length,
